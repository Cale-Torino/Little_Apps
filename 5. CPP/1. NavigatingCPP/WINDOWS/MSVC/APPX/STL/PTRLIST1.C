// ptrlist1.C - STL list container with pointer semantics
#include <iostream>
#include <list>
#include "Shape.h"
...
void build_list(list<Shape *> &);
void shape_info(const list<Shape *> &);

//template <class Container>
//void destroy_objects(Container &);
void destroy_objects(list<Shape *> &);

int main() 
{
	list<Shape *> shapelist;						// pointer List
	...
	cout << "There are " << Shape::num_shapes() 
			<< " shapes left" << endl;
	destroy_objects(shapelist);					// delete Shape pointers
	return 0;
}

void build_list(list<Shape *> & ls) {
...
void shape_info(const list<Shape *> & ls) {
	list<Shape *>::const_iterator iter = ls.begin();
...
void destroy_objects(list<Shape *> & c) {
	list<Shape *>::iterator iter = c.begin();
	for (; iter != c.end(); iter++)
		delete *iter;
}
