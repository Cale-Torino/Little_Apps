...
private:
	int num;
public:
	GridError(int n = 0) throw();						// OK, no throws
	virtual void response() const throw (); 		// OK, no throws
};

GridError::GridError(int n) throw() : num(n) { }

class ChartError : public GridError {
public:
	ChartError(int m = 0) throw();					// OK, no throws
	void response() const throw (); 					// OK, no throws
};

ChartError::ChartError(int m) throw() : GridError(m) { }
