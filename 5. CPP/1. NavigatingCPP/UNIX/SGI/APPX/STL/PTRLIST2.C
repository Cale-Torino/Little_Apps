// ptrlist2.C - STL list container with pointer semantics, Shape class destroy
#include <iostream.h>
#include <list.h>
#include "Shape.h"
...
class PtrListShape : public list<Shape *> {
public:
	PtrListShape() { }
	~PtrListShape() { destroy_objects(*this); }
};

void build_list(PtrListShape &);
void shape_info(PtrListShape &);

int main() 
{
	PtrListShape shapelist;
...
void build_list(PtrListShape & ls) {
...
void shape_info(PtrListShape & ls) {
	PtrListShape::const_iterator iter = ls.begin();
