...
int main()
{
	...
	int i;
	for (i = 0; i < len; i++)
		if (numbers[i] < 0) a.push_front(numbers[i]);
		else a.push_back(numbers[i]);
	for (i = 0; i < a.size(); i++)
		cout << a[i] << ' ';
	cout << endl;
	...
	int neg = 0;
	count_if(a.begin(), a.end(), negative, neg);
	cout << "There are " << neg << " negative values in container a" << endl;
	for_each(a.begin(), a.end(), display_times_ten);
	cout << endl;
	return 0;
}
