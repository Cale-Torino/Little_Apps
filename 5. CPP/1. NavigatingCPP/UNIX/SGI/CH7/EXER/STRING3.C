// String3.C - String class implementation, different istream version
#include <iostream.h>
#include <iostream.h>
#include <stdlib.h>
#include <string.h>
...
istream & operator>>(istream & is, String & s1) {			// NEW
		const int delta = 256;
		unsigned size = delta;
		char *p, *q;
		p = q = new char[size];
		for (int nc = 1; (*p = is.get()) != '\n' && !is.eof(); nc++) {
		if (nc % delta == 0) {
			size += delta;
		...
		*p = '\0';
		s1 = q;							// String assignment and constructor
		delete [] q;
		return is;
}
